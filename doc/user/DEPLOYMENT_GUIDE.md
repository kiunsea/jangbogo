...
> **대부분의 경우 위 원스톱 절차면 충분합니다.** 아래 섹션은 직접 빌드하거나, 특수한 경로/설정으로 수동 설치·운영하려는 **고급 사용자·문제 해결용** 참고 자료입니다.
...
### 🚀 표준 절차
- 상단 [원스톱 설치](#-원스톱-설치-v070-권장) — 대부분 이것만 있으면 됩니다

### 🔧 고급 / 수동 절차
1. [빌드 방법](#빌드-방법) — 소스에서 직접 빌드
2. [배포 패키지 구성](#배포-패키지-구성) — ZIP 내부 구조 참고
3. [수동 설치](#수동-설치) — 원스톱 대신 직접 설치
4. [수동 실행](#수동-실행) — `install.bat` 대신 `Jangbogo.bat` 직접 기동
5. [관리자 계정 설정](#관리자-계정-설정)
6. [수동 Windows 서비스 등록](#수동-windows-서비스-등록) — `install.bat` 미사용 시
7. [수동 제거](#수동-제거)
...
> 💡 배포 ZIP 폴더에서 **관리자 권한으로 `install.bat`** 한 번 실행하면 이 섹션의 모든 단계가 자동화됩니다. 아래는 원스톱 스크립트를 쓰지 않고 직접 제어하려는 경우의 절차입니다.
...
> 💡 원스톱 `install.bat` 이 서비스로 자동 기동 + 브라우저 오픈 + 트레이까지 처리합니다. 이 섹션은 서비스 등록 없이 포그라운드로 직접 기동하려는 경우에 해당합니다.
...
<a id="수동-windows-서비스-등록"></a>
## 수동 Windows 서비스 등록

> 💡 원스톱 `install.bat` 은 WinSW 등록 + JAR 경로 자동 동기화 + `RUNNING` 상태 폴링 + 로그 tail 까지 수행합니다. 아래 절차는 스크립트를 쓰지 않고 직접 `jangbogo-service.exe` 를 호출하려는 경우에만 필요합니다.
...
> 💡 배포 ZIP 폴더에서 **관리자 권한으로 `uninstall.bat`** 실행이 표준 제거 방법입니다. 서비스 제거 + 단축아이콘 삭제 + 트레이 프로세스 종료가 한번에 수행되며, `db/` `logs/` `exports/` 는 보존됩니다. 아래 절차는 `uninstall.bat` 을 쓰지 못하는 환경에서의 수동 대안입니다.

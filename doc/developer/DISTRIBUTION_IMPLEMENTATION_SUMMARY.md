...
| 사용자 매뉴얼 제공 | ✅ | 사용설명서.txt, 설치가이드.txt, 고급가이드.txt, 사용자_매뉴얼.txt |
...
│   - 관리자 계정 변경 방법 (3가지)
│   - 문제 해결
├── 설치가이드.txt                  [NEW] 상세 설치 방법
│   - 설치 절차, 관리자 계정 변경, admin.properties 등
├── 고급가이드.txt                  [NEW] config 폴더 시스템 설정 가이드 (고급 사용자용)
│   - admin.properties, jbg_config.yml, mall_account.yml, application.yml 설명
...
4. **완전한 문서**
   - 설치·설정 가이드 (`사용설명서.txt`, `설치가이드.txt`)
   - config 폴더 설정 가이드 (`고급가이드.txt`)
   - 사용 가이드 (`사용자_매뉴얼.txt`)
   - README.md 포함

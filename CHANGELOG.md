...
Phase 5 선행 코드 이어서. 여전히 기능은 꺼져 있고 동작은 바뀌지 않습니다.

### Added

- **`BrowserConcurrencyLimiter`** (5-11) — 브라우저 동시 실행 수를 이 JVM 안에서 제한합니다. 기본 **1개**.
  - `MallProfileLock` 과 역할이 다릅니다. 락은 **프로필 하나**를 **프로세스 사이**에서 지키고(같은 프로필을 두 Chrome 이 열면 세션이 깨집니다), 이 제한은 **브라우저 총 개수**를 묶습니다. 프로필이 서로 달라도 Chrome 을 여럿 띄우면 메모리·CPU 가 무너지고, 같은 시각에 여러 몰로 나가는 트래픽은 그 자체가 눈에 띄는 패턴입니다.
  - **기다리는 정책이 락과 반대입니다.** 프로필 락은 상대가 다른 프로세스라 언제 끝날지 몰라 즉시 물러나지만, 이 제한은 상대가 같은 JVM 의 수집이고 몇 분이면 끝난다는 것을 압니다. 즉시 포기하면 그 몰은 **다음 주기(12시간)까지 통째로 건너뜁니다.**
  - 대기 상한 기본값은 **300초**입니다. 실측 수집 한 회차가 약 2분 30초라, 앞선 수집을 기다리기엔 넉넉하고 멈춘 수집에 무한정 매달리지는 않는 값입니다. 넘기면 `SKIPPED/BROWSER_BUSY` 로 물러납니다.
  - 공정(fair) 세마포어입니다. 먼저 기다린 몰이 먼저 받습니다 — 아니면 한 몰이 계속 밀립니다.
  - `jangbogo.browser.concurrency` / `jangbogo.browser.acquire-timeout-sec` 로 조정합니다.

### Added (테스트)

- `BrowserConcurrencyLimiterTest`(10건): 획득·해제, 대기 상한까지 기다렸다가 물러남, **앞선 수집이 끝나면 대기하던 쪽이 자리를 받음**(멀티스레드), 허용 수 초과 거부, 실패한 임대권을 닫아도 자리가 늘지 않음, 이중 close 안전, 설정 기본값·재정의·방어. 총 **285건**.
...
- **개발용 엔드포인트가 운영 JAR 에 실려 있던 문제 (B-2)**: `/dev/reset-database` 는 주문·아이템을 통째로 지우는데 프로파일 가드가 없어 배포본에 그대로 들어갔습니다. 인증이 앞을 막고 있었지만, 파괴적 기능이 운영 산출물에 있는 것 자체가 문제입니다. `dev` 패키지의 빈 4개에 `@Profile("dev")` 를 붙였습니다. **`--spring.profiles.active=dev` 로 띄우지 않으면 등록되지 않습니다.**
- **암호화 키가 소스에 박혀 있던 문제 (B-1)**: 이 저장소는 PUBLIC 이라 키가 이미 공개돼 있습니다. `jangbogo.crypto.key` / `jangbogo.crypto.iv` 시스템 프로퍼티(또는 `JANGBOGO_CRYPTO_KEY` 환경변수)로 덮어쓸 수 있게 했고, 기본값을 쓰면 기동 시 경고를 남깁니다.
  - **기본값은 바꾸지 않았습니다.** 바꾸는 순간 기존 DB 의 FTP 비밀번호를 복호화할 수 없게 됩니다. 재암호화 마이그레이션은 별도 과제입니다.
- **SQL 문자열 조립 (B-3)**: `serial_num`·`mall_name`·상품명은 수집한 웹페이지에서 온 값이라 따옴표 하나로 INSERT 가 깨지고 그 자리가 주입 지점이 됩니다. 같은 INSERT 가 두 벌 있었고 한쪽만 고쳐져 있었습니다 — 수집기가 쓰는 `addWithConnection` 은 진작 PreparedStatement 였고 `add(...)` 오버로드만 조립으로 남아 있었습니다. 구현을 하나로 모았습니다.
- **`/actuator` 인증 예외 제거 (B-4)**: actuator 의존성이 없어 지금은 404 지만, 나중에 넣으면 `health`·`env`·`beans` 가 무인증으로 열린 채 시작합니다. 쓰지도 않는 예외를 미리 깔아 두지 않습니다.

### Fixed

- **수집기 건강도 배너가 API 실패 시 이전 내용을 그대로 두던 문제**: 응답을 못 받았는데 직전 경고가 남아 있으면 "지금도 문제가 있다"로 읽힙니다. 실패·파싱오류·네트워크오류 분기가 전부 배너를 비웁니다. 주기 갱신을 붙이면 실제로 발생할 오독이었습니다.
- **배포본 `README.md` 의 버전 드리프트**: 제목과 폴더 구조 설명이 `v0.8.1` 을 가리키고 있었습니다. WinSW XML·`clean_build.bat` 과 같은 계열이라 같은 방식(버전 미기재)으로 고쳤습니다.

### Added (테스트)

- `SecurityHardeningTest`(3건) · `PasswordEncryptorKeyTest`(5건): 프로파일 가드, 키 재정의와 **기본값 폴백**(기존 암호문이 계속 열려야 한다), 암복호 왕복, SQL 조립 금지, actuator 예외 금지. 총 **260건**.
...
- **개편된 영수증 화면에서 구매내역이 통째로 버려지던 문제**: 이마트가 영수증 화면을 개편해 `ern-` 접두 템플릿이 섞여 내려오는데, 그 템플릿에는 기존 `#barcodeTargetRec` 이 없어 바코드를 못 읽고 그 영수증이 저장 단계에서 버려졌습니다(17건 중 2건). 이제 세 경로를 차례로 시도합니다.
  - **A** — 구 템플릿 `#barcodeTargetRec` (기존 동작, 그대로)
  - **A2** — 신 템플릿의 바코드 요소 `#barcode-no` / `#ern-barcode`
  - **B** — 분해된 식별 필드 `biz_date` + `pos_str_code` + `pos_no` + `tran_no` 로 `serial` 조립
  - 셋 다 실패하면 지금처럼 버리되, 각 경로가 왜 실패했는지 남깁니다.

### Changed

- **경로 C(본문 날짜)는 저장에 쓰지 않습니다.** 본문 10줄째에 거래일시가 있는 것은 확인했지만, 중복 판정이 `serial_num` + `date_time` **조합**이라 `serial` 없이 저장하면 **같은 날 산 영수증이 한 건으로 뭉개집니다** — 첫 건만 남고 나머지는 "기존 주문"으로 스킵됩니다. 저장 이력에 같은 날짜 3건이 실재하므로 가상의 위험이 아닙니다. 그래서 진단으로만 남깁니다.
- 같은 이유로 **경로 B 도 매장·POS·거래번호가 하나도 없으면 `serial` 을 만들지 않습니다.** 날짜만으로는 같은 날 영수증을 구분할 수 없습니다. 틀린 키로 저장해 데이터를 잃는 것보다 버리는 편이 낫습니다.
- 복구에 성공하면 어느 경로였는지와 **자릿수**를 남깁니다. 값은 남기지 않습니다.

### Added (테스트)

- `EmartSerialFallbackTest`(14건): 필드 조합, 구분자 섞인 날짜, **꼬리 없는 날짜-only serial 거부**, 같은 날 영수증 구별, 재수집 안정성, 날짜 범위 위반 거부, 본문 날짜 추출, 경로 대조, 그리고 진단 문자열에 값이 새지 않는다는 경계.

### Notes

- **실계정 수집으로 검증했습니다: 17건 수집 → 17건 저장, 스킵 0건**(이전 2건). 기존 15건은 중복으로 정확히 판정되어 재수집이 중복 주문을 만들지 않음도 함께 확인했습니다. 미인식 2건은 모두 **경로 B**로 복구됐고 A2 는 실패했습니다 — `#barcode-no`·`#ern-barcode` 에는 바코드 형식을 만족하는 값이 없었습니다. 상세는 `DEVLOG.md` 의 `[2026-08-02 16:55]`.

---

## [0.13.8] - 2026-08-01

바코드 미인식 영수증 2차 진단. `#barcodeTargetRec` 이 없다는 것까지 확정됐으므로, 이제 **그 자리에 무엇이 있는지**와 **본문에서 날짜를 얻을 수 있는지**를 함께 봅니다. (판단 대기 B-3 후속)

### Added

- **팝업 구조 census**: 문서 전체의 태그 집계와 id 목록을 남깁니다. 대체 컨테이너를 찾으면 셀렉터를 고칠 수 있습니다. 요소마다 왕복하면 수백 번이 되므로 JS 한 번으로 모읍니다.
  - **id 의 숫자는 가립니다.** id 는 대개 템플릿 식별자지만 `order_20991231123` 처럼 주문번호가 섞이는 형태가 있을 수 있어, 숫자를 `N` 으로 바꿔 형식만 남깁니다. 같은 형태로 반복되는 행 id 는 한 종으로 묶고, 40종을 넘으면 잘라냅니다.
- **본문 날짜 패턴 판정**: 영수증 본문에 구매일시가 있는지, 있다면 어떤 **형식**인지 보고합니다. 6종(`YYYY-MM-DD` / `YYYY/MM/DD` / `YYYY.MM.DD` / `YYYY년MM월DD일` / 8연속숫자 / `HH:MM`)의 매치 수와 첫 줄 번호만 남깁니다.
  - **날짜 값도 본문 줄도 싣지 않습니다.** 필요한 것은 "뽑을 수 있는가, 어떤 형식인가"이고 날짜 값 자체는 구매 이력입니다. 줄을 그대로 싣는 방법도 쓰지 않습니다 — 숫자를 가려도 같은 줄의 상품명이 남기 때문입니다.
  - 이 판정이 되면 `serial`·`datetime` 을 바코드에 의존하지 않아도 되어 결함이 **해결**됩니다.

### Added (테스트)

- `EmartBarcodeProbeTest` 9건 추가(총 20건): id 숫자 마스킹, 동형 id 병합, 목록 상한과 말줄임, 본문 부재, 날짜 형식·위치 보고, 날짜 없음, 쉼표 있는 금액을 날짜로 오인하지 않음, 형식별 개별 집계, 그리고 **요약에 날짜 값·본문 내용·라벨이 새지 않는다는 경계**.

### Notes

- 실측 결과 미인식 영수증은 **`ern-` 접두 id 를 쓰는 새 템플릿**이었습니다. 이마트가 영수증 화면을 개편했고 구·신 템플릿이 같은 회차에 섞여 내려옵니다. 신 템플릿에는 `#barcode-no`·`#ern-barcode` 가 있고, 영수증 식별 필드가 `biz_date`·`pos_str_code`·`pos_no`·`tran_no` 로 분해돼 있습니다. 본문에도 거래일시가 정확히 1건 있습니다. **수집을 되살릴 경로가 확인됐습니다** — 상세와 수정 계획은 `DEVLOG.md` 의 `[2026-08-02 03:45]`.

---

## [0.13.7] - 2026-08-01

바코드 미인식 영수증의 원인을 가리기 위한 진단 릴리스. (판단 대기 B-3 후속)

### Added

- **바코드 미인식 시 구조 프로브**: 실측에서 원인이 `#barcodeTargetRec` 안에 div 가 0개인 것으로 좁혀졌지만, 그것만으로는 세 갈래를 구분할 수 없었습니다 — 바코드가 canvas/svg/이미지로 **그려지는** 것인지, 컨테이너가 정말 **비어 있는** 것인지, 대기가 짧아 **아직 안 그려진** 것인지. 이제 못 읽었을 때만 컨테이너의 **구조**를 두 시점(읽은 직후, +1.5초)에 떠서 경고 로그에 함께 남깁니다.
  - 남기는 것: 컨테이너 존재 여부, class 속성, 부모 태그명, 자손 태그 집계(`canvas=1, div=2` 형태), 텍스트 **길이**.
  - **구매 정보는 담지 않습니다.** 이 저장소는 PUBLIC 이고 로그는 배포본에 남으므로, 텍스트는 내용이 아니라 길이만 셉니다. 상품명·금액·구매일시가 들어갈 자리가 시그니처에 없습니다.
  - 진단이 수집을 깨뜨리면 원래 결함보다 나쁘므로, 프로브는 어떤 예외도 밖으로 내보내지 않습니다. 태그명을 못 읽으면 `?` 로 세고, 텍스트를 못 읽은 것과 0자를 구분합니다.

### Added (테스트)

- `EmartBarcodeProbeTest`(11건): 컨테이너 부재, 빈 컨테이너, canvas 렌더링, 태그 집계 정렬·대소문자 정규화·판독 불가 태그, 텍스트 읽기 실패와 0자 구분, 부모/자손 결측 내성, 그리고 **요약에 구매 정보가 새지 않는다는 경계**. 순수 함수라 브라우저를 띄우지 않습니다.

### Notes

- 실측 결과 미인식 2건은 **`#barcodeTargetRec` 요소 자체가 없는** 팝업이었습니다. canvas 렌더링도, 빈 컨테이너도, 렌더링 지연도 아닙니다. 같은 회차의 나머지 15건은 같은 셀렉터로 정상 인식되므로, 사이트가 영수증에 따라 다른 DOM 을 내려주고 있습니다. 수집 자체는 아직 해결되지 않았습니다 — 상세는 `DEVLOG.md` 의 `[2026-08-01 22:30]`.
...
- **바코드를 못 읽으면 영수증이 조용히 사라지던 문제**: `#barcodeTargetRec div` 가 1개 이하면 바코드가 `null` 이 되고, 그 영수증은 `serial`·`datetime` 없이 수집되어 저장 단계에서 말없이 버려집니다(`skippedOrders` 로만 세어짐). 이제 못 읽으면 **경고 로그로 남깁니다** — 왜 버려졌는지 알 수 있습니다.
- **빈 바코드가 수집 전체를 무너뜨릴 수 있던 문제**: 마지막 div 의 텍스트가 비었거나 8자 미만이면 `receiptBarcode.substring(0, 8)` 이 `StringIndexOutOfBoundsException` 을 던집니다. **영수증 하나 때문에 Emart 수집 전체가 실패**합니다. 길이·형식 검증을 통과한 값만 쓰도록 했습니다.
- **"마지막 div" 라는 위치 가정**: 렌더링 구조에 대한 가정일 뿐이라, 뒤에 라벨이나 래퍼 div 가 하나 붙으면 **엉뚱한 값이 `serial` 로 저장**됩니다. 이제 뒤에서부터 훑되 **바코드처럼 생긴 것만** 받습니다.

### Changed

- **바코드 판정 기준**: 호출측이 앞 8자리를 `YYYYMMDD` 로 쓰므로, 그 용도가 성립하는지를 그대로 기준으로 삼습니다 — 공백 제거 후 8자 이상, 앞 8자가 숫자, 그 8자가 실제 날짜 범위(월 01~12, 일 01~31). 휴대폰 번호나 금액 같은 긴 숫자가 바코드로 잘못 잡히지 않습니다.
- **못 찾으면 추측하지 않고 `null`** 을 돌려줍니다. 틀린 `serial` 로 저장하는 것보다 낫습니다.
- 바코드 텍스트의 **공백을 제거**합니다. 앞에 공백이 붙어 있으면 `substring(0, 8)` 이 엉뚱한 `datetime` 을 만들었습니다.
- div 하나를 못 읽어도 나머지 후보는 계속 봅니다.

### Added (테스트)

- `EmartBarcodeTest`(10건): 라벨 div 가 뒤에 붙은 경우, 후보가 하나뿐인 경우, 빈 문자열·8자 미만·비숫자·날짜 범위 초과, `null` 원소 혼재. 순수 함수라 브라우저를 띄우지 않습니다.

---

## [0.13.4] - 2026-08-01

수집 한 회차가 6분간 붙잡히던 원인과, 그때 멀쩡한 데이터가 버려지던 문제를 함께 고친 릴리스. (판단 대기 B-2)

### Fixed

- **페이지 로드 타임아웃이 설정된 적이 없어 Selenium 기본값 300초가 적용되던 문제**: 실측에서 Emart 트레이더스 영수증 페이지가 응답하지 않자 수집 한 회차가 **약 6분**(준비 45초 + 300초)을 통째로 붙잡혔습니다. 실패 시점 capabilities 에 `timeouts={implicit:0, pageLoad:300000, script:30000}` 로 찍혀 있었습니다.
  - 기본값을 **60초**로 명시했습니다. 정상 페이지에 넉넉하고, 죽은 페이지는 6분이 아니라 1분 안에 드러납니다.
  - 느린 회선을 만나면 `-Djangbogo.browser.page-load-timeout-sec=120` 으로 올릴 수 있습니다.
- **한쪽 영수증이 죽으면 다른 쪽 결과까지 버려지던 문제**: `Emart.navigateReceipt` 가 이마트(E)와 트레이더스(T)를 한 줄로 이어 호출해서, T 에서 예외가 나면 **이미 모아 둔 E 결과가 통째로 사라졌습니다.** `resJsonArr` 이 반환되지 못하기 때문입니다.
  - 이제 둘을 각각 감싸 한쪽 실패가 다른 쪽을 막지 않습니다. **둘 다 실패했을 때만** 수집 실패로 올립니다.
  - `MallOrderUpdater` 가 SSG·Emart 사이에 이미 적용해 둔 부분 실패 격리와 같은 원칙입니다 — 그게 한 겹 아래에는 빠져 있었습니다.

### Added (테스트)

- `WebDriverOptionsTest` +2건 — 타임아웃이 300초가 아닌 60초로 박히는지, 시스템 프로퍼티로 조정되고 잘못된 값은 무시되는지.
- `EmartReceiptIsolationTest`(4건) — 트레이더스가 죽어도 예외가 안 올라가는지, 이마트가 죽어도 **트레이더스가 반드시 시도되는지**, 둘 다 죽으면 첫 실패가 올라가는지. 브라우저를 띄우지 않습니다.

### Known limitation

- 한쪽만 실패한 경우 그 사실은 **애플리케이션 로그에만** 남고 `jbg_collect_log` 에는 수집기(Emart) 단위 한 행으로만 기록됩니다. E·T 를 따로 세려면 별도 수집기로 분리해야 하는데, 둘은 로그인 세션을 공유하므로 그러면 로그인이 두 번 일어납니다. 현재는 의도적으로 로그 노출까지만 합니다.
- **트레이더스 페이지가 왜 무응답이었는지는 규명하지 못했습니다.** 이번 수정은 원인과 무관하게 피해를 줄입니다.
...
- **`last_export_time` 이 한 번도 갱신되지 않던 문제**(판단 대기 10): 조사해 보니 갱신만 안 되는 게 아니라 **읽지도 쓰지도 표시하지도 않는 완전한 죽은 컬럼**이었습니다. `schema.sql` 에 선언만 있고 자바 코드 어디에서도 참조하지 않았고, 설정 조회 SELECT 에도 빠져 있었습니다. 로컬 내보내기 파일이 11개나 쌓여 있는데도 값이 비어 있던 이유가 그것입니다.
  - 이제 파일 저장과 FTP 전송이 성공할 때마다 기록합니다.
  - 기록 실패는 내보내기를 되돌리지 않습니다 — 경고만 남기고 넘어갑니다.

### Added

- **`last_ftp_upload_time` 컬럼**: FTP 전송 성공 시각을 **로컬 저장과 따로** 셉니다. 두 경로는 각각의 설정 플래그(`auto_save_enabled` / `save_to_jiniebox`)로 켜고 끌 수 있어 한쪽만 도는 구성이 정상입니다. 하나의 시각으로 뭉치면 **"파일은 저장되는데 전송만 죽은" 상태를 구분할 수 없습니다.**
  - 보류 큐의 재전송 성공도 갱신합니다. 빼면 밀린 것만 나가는 기간 동안 마지막 전송 시각이 멈춰 보입니다.
- **대시보드 표시**: '구매정보 저장 옵션' 카드의 저장 위치 아래에 '마지막 저장'과 '마지막 전송'을 나란히 보여 줍니다. 기록이 없으면 그렇게 적습니다.

### Changed

- 로컬 저장 시각 기록은 **파일을 실제로 쓴 `ExportService` 메서드 안**에 뒀습니다. 컨트롤러·스케줄러에 호출을 흩지 않으므로 저장 경로가 늘어나도 기록이 빠지지 않습니다. FTP 전송 준비용 임시 파일은 여기 해당하지 않습니다 — 올린 뒤 지우는 파일이라 "사용자가 보관하는 내보내기"가 아닙니다.

### Added (테스트)

- `ExportTimestampTest`(7건): 두 시각이 서로를 덮지 않는지, 설정 조회에 함께 실려 나오는지(쓰기만 하고 조회에 안 실으면 다시 죽은 컬럼이 됩니다), 설정 행이 없어도 예외를 밖으로 내지 않는지.

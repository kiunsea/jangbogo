...
계획서의 "AGPL 대응 소스 의무와 직결" 이라는 프레이밍은 **낡은 것이었다.** 그 표현은 *PRIVATE doribox 코드가 PUBLIC 릴리스 ZIP 에 들어가는* 상황을 전제한 것인데, doribox import 가 폐기되면서 그 경로 자체가 사라졌다.
...
- Edge 타임아웃 부재는 전파 대장 후보(드라이버 옵션 조립·전달) — 대장 P3-1 등재. doribox 4개 생성 지점의 타임아웃 설정 여부는 미확인.

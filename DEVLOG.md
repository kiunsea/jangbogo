...
`MallOrderUpdaterRunner:325` 가 `nothingUsable ? "FAIL" : "SUCCESS"` 로 판정한다. seq=1 은 수집기가 SSG·Emart **둘**인데 결과는 `jbg_collect_log` **한 행**으로 합쳐진다.

⇒ **SSG 가 매번 실패해도 Emart 가 성공하면 그 회차는 SUCCESS 로 기록된다.** 실제로 `Ssg.signin()` 이 하드코딩 `false` 였던 기간 내내 그랬다. 연속 실패를 몰 단위로 세는 브레이커는 SSG 단독 장애를 영원히 트립하지 못한다.

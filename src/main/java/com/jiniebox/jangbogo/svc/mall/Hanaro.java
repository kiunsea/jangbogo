    ...
    try {
      if (driver == null) {
        throw CollectStep.wrap(
            null, "HANARO", "init-webdriver", null, new IllegalStateException("WebDriver 생성 실패"));
      }
      boolean signedIn = CollectStep.call(driver, "HANARO", "signin", () -> this.signin(driver));
      if (!signedIn) {
        throw CollectStep.wrap(
            driver,
            "HANARO",
      ...
      resArr =
          CollectStep.call(
              driver, "HANARO", "navigatePurchased", () -> this.navigatePurchased(driver));

      // 마무리
      try {
        this.signout(driver);
      } catch (Exception ignore) {
        log.warn("Hanaro 로그아웃 중 오류(무시): {}", ignore.getMessage());
      }
    } finally {
      try {

...
public interface SessionCollector {
  ...
  enum SkipCause {
    ...
    /**
     * 그 프로필을 다른 프로세스가 쓰고 있어 이번 회차를 돌지 않았다 ({@code MallProfileLock} 배선).
     *
     * <p><b>{@link #NO_SESSION} 으로 뭉뚱그리지 않는다.</b> 저쪽 사유 문구는 "대시보드에서 '브라우저로 로그인' 을 먼저 실행할 것" 인데, 세션은
     * 멀쩡히 있고 프로필만 겹친 이 상황에서 그 안내를 읽은 사람은 <b>아무 효과도 없는 재로그인</b>을 반복하게 된다. 종류가 갈려 있어야 화면이 다른 말을 할 수
     * 있다.
     *
     * <p>승격 대상이 아니다 — 세션이 죽은 것이 아니라 잠깐 겹친 것뿐이라, 이것으로 몰을 일시중단하면 다음 회차에 저절로 풀릴 일이 사람 손을 기다리게 된다.
     */
    PROFILE_LOCKED,

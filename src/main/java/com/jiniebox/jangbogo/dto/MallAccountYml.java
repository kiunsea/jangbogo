...
public class MallAccountYml {
  ...
  public Optional<MallAccount> getAccountBySite(String site) {
    return accounts.stream().filter(account -> site.equals(account.getSite())).findFirst();
  }
  ...
  /** 전체 계정 수 JsonIgnore로 직렬화 제외 */
  @com.fasterxml.jackson.annotation.JsonIgnore
  public int getAccountCount() {
    return accounts.size();
  }

  /** 모든 사이트명 목록 조회 JsonIgnore로 직렬화 제외 */
  @com.fasterxml.jackson.annotation.JsonIgnore
  public List<String> getAllSites() {
    return accounts.stream().map(MallAccount::getSite).toList();
  }

  @Override
  public String toString() {
    return "MallAccountYml{" + "accounts=" + accounts.size() + " accounts" + '}';
  }
}

 ...
 *
 * <p>Main 함수로 직접 실행 가능한 FTP TLS 연결 및 파일 업로드 테스트 프로그램입니다.
 *
 ...
 * </pre>
 *
 * <p>또는 환경 변수 사용 (우선순위: 환경변수 > 프로퍼티 파일 > 기본값)
 *
 * @author jiniebox.com
 * @version 1.0
 * @since 2025-11-05
...
public class SimpleFtpTlsUploadTest {
  ...
  public static void main(String[] args) {
    ...
    // 프로퍼티 파일 경로들
    String[] propsPaths = {"src/test/resources/ftp-test.properties", "ftp-test.properties"};
    ...
    System.out.println(
        "  - Host: "
            + ftpHost
            + " (출처: "
            + getSource(System.getenv("FTP_HOST"), props.getProperty("ftp.host"))
            + ")");
    ...
    System.out.println(
        "  - Port: "
            + ftpPort
            + " (출처: "
            + getSource(System.getenv("FTP_PORT"), props.getProperty("ftp.port"))
            + ")");
    ...
    System.out.println(
        "  - User: "
            + ftpUser
            + " (출처: "
            + getSource(System.getenv("FTP_USER"), props.getProperty("ftp.user"))
            + ")");
    ...
    System.out.println(
        "  - Pass: "
            + maskPassword(ftpPassword)
            + " (출처: "
            + getSource(System.getenv("FTP_PASS"), props.getProperty("ftp.password"))
            + ")");
    ...
    System.out.println(
        "  - 전송 모드: "
            + (usePassiveMode ? "PASSIVE (수동형)" : "ACTIVE (능동형)")
            + " (출처: "
            + getSource(System.getenv("FTP_MODE"), props.getProperty("ftp.mode"))
            + ")");
    ...
    try {
      ...
      if (files.length > 0) {
        ...
        for (int i = 0; i < Math.min(5, files.length); i++) {
          ...
          System.out.println(
              "    " + type + " " + file.getName() + " (" + file.getSize() + " bytes)");
        }
        if (files.length > 5) {

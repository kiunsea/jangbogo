 ...
 *
 * <p>이 테스트는 FTPS 서버 연결, 인증, 파일 목록 조회, 파일 업로드/다운로드 등을 테스트합니다.
 *
 * <p><strong>주의:</strong> 실제 FTP 서버 정보가 필요하므로 기본적으로 @Disabled 처리되어 있습니다. 테스트를 실행하려면:
 *
 * <ul>
 *   <li>1. 환경 변수 또는 테스트 프로퍼티 파일에 FTP 서버 정보 설정
 *   <li>2. @Disabled 어노테이션 제거
 * </ul>
 *
 * <p>환경 변수 설정 예시:
 *
...
class FtpTlsConnectionTest {
  ...
  private java.util.Properties loadProperties() {
    ...
    String[] paths = {"src/test/resources/ftp-test.properties", "ftp-test.properties"};
  ...
  void testBasicFtpsConnection() throws Exception {
    ...
    assertTrue(FTPReply.isPositiveCompletion(reply), "FTP 서버가 연결을 거부했습니다. Reply: " + reply);
  ...
  void testUploadFile() throws Exception {
    ...
    String testContent =
        "This is a test file uploaded via FTPS at " + java.time.LocalDateTime.now();
   ...
   *
   * <p><strong>경고:</strong> 프로덕션 환경에서는 사용하지 마세요! 이 방법은 중간자 공격(MITM)에 취약합니다.
   */
